...
window.onload = function () {
  ...
  function navPathDataBind() {
    /*思路： 
      ...
      3.由于数据是需要动态产生的，那么相应的DOM元素也应该是动态产生的，含义需要创建DOM元素，根据数据的数量进行创建DOM元素
  ...
  function bigClassBind() {
    ...
    smallPic.onmouseenter = function () {
      ...
      smallPic.onmousemove = function (event) {
        ...
        // 移动放入比例关系 = 蒙版元素移动的距离 / 大图片元素移动的距离
        // 蒙版元素移动的距离 = 小图片宽度 - 蒙版元素的宽度
  ...
  function thumbnailClick() {
    ...
    */
    ...
    imagessrc.src = imagessrc[0].s;

    // 2.循环点击li元素事件
    for (var i = 0; i < liNodes.length; i++) {

      // 在点击事件之前,给每一个元素都添加自定义的下标
      liNodes[i].index = i;
  ...
  function thumbnailLeftRightClick() {
    ...
    */
    ...
    //  总体运动的距离值 = ul的宽度 - div值的宽度 = （图片的总数 - div中显示的数量）* （li的宽度 + 20）
  ...
  function rightTopDate() {
    ...
    思路：
    1.查找rightTop元素
    2.获取data.js文件下的goodData-> goodsDetail数据
    3.建立一个字符串变量，将原来的布局机构贴进来，将所对应的数据放在对应的位置上重新渲染rightTop元素
    */
    //  1.查找rightTop元素
  ...
  // 商品详情中间  选择搭配中间区域复选框选中套餐价变动效果
  choosePrice();
  function choosePrice() {
    /* 
    思路：
    1.先获取中间元素的复选框元素
    2.遍历这些元素取出的价格，和左侧的基础价进行累计，累计之后重新计算 
    */
